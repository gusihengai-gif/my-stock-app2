...
# --- 1. 原始台股清單資料池 (這裡可以自由擴充) ---
# 確保維持純淨的代碼與名稱對應，不進行任何混亂的分組
ALL_STOCKS = {
    "2330": "台積電",
    "2317": "鴻海",
    "2454": "聯發科",
...
# 渲染出標準型態的 selectbox（可以手打字、可以往下滑、外觀100%符合你最後上傳的截圖）
...
# 最終提煉出要拿去 Yahoo Finance 抓真實報價的 4 位或多位股票數字代碼
...
if final_target_code:
    ...
    if not data.empty:
        ...
        # 針對不同天數區間自動分配 x 軸刻度間隔，確保手機或小螢幕看也不會擠成一團字
        tick_vals = []
        tick_texts = []
