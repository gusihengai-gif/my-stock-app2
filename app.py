# --- 5. 核心：單一搜尋欄 (字首強鎖定 + 打字即時連動免按 Enter) ---
st.sidebar.title("🚀 股票買賣時機")

# 1. 建立一個方便快速查找的對照清單 (例如: "2330 台積電")
...
# 5. 用唯一一個 selectbox 完美兼顧「打字即時動態連動」與「強字首鎖定」
# 透過 label 內放入隱藏狀態引導使用者
selected_stock_str = st.sidebar.selectbox(
    "請輸入或選擇股票代碼：",
    options=ALL_STOCKS_LIST,
    index=list(ALL_STOCKS.keys()).index(st.session_state.final_target_code) if st.session_state.final_target_code in ALL_STOCKS else 0
)

# 6. 當使用者在手機上打字或選好時，即時擷取最前面的純代碼 (免按 Enter 機制)
if selected_stock_str:
    chosen_code = selected_stock_str.split(" ")[0].strip()
    
    # 這裡進行二次鋼鐵把關：只有當使用者真正選定或完全匹配時才寫入，確保穩定
    st.session_state.final_target_code = chosen_code
